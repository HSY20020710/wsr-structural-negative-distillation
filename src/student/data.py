...
def make_random_counterfactual_target(
    record: dict,
    ontology: dict,
    rng: random.Random | None = None,
) -> dict | None:
    """Build one relation corruption without consulting WSR signatures."""
    rng = rng or random.Random(0)
    target = canonical_target(record)
    if not target["relations"]:
        return None
    negative = json.loads(json.dumps(target, ensure_ascii=False))
    relation_index = rng.randrange(len(negative["relations"]))
    relation = negative["relations"][relation_index]
    entity_ids = [entity["id"] for entity in negative["entities"]]
    relation_types = [
        item
        for item in ontology.get("relation_types", [])
        if item != "NO_RELATION" and item != relation["type"]
    ]
    operations = []
    if relation_types:
        operations.append("relation_type")
    for endpoint in ("head", "tail"):
        if any(entity_id != relation[endpoint] for entity_id in entity_ids):
            operations.append(endpoint)
    if not operations:
        return None
    operation = rng.choice(operations)
    if operation == "relation_type":
        relation["type"] = rng.choice(relation_types)
        counterfactual_type = "random_relation_type"
    else:
        relation[operation] = rng.choice(
            [entity_id for entity_id in entity_ids if entity_id != relation[operation]]
        )
        counterfactual_type = f"random_{operation}_replacement"
    return {
        "counterfactual_type": counterfactual_type,
        "negative_source": "random_counterfactual",
        "target": negative,
    }
...
def _as_training_items(
    ...
    negative_policy: str = "full",
) -> list[dict]:
    if negative_policy not in {"full", "none", "synthetic_only", "random"}:
        raise ValueError(f"Unsupported negative_policy: {negative_policy}")
    if target_schema == "text":
        need_negative = False
    if negative_policy == "none":
        need_negative = False
    gate_negatives = (
        make_gate_rejected_targets(record, ontology)
        if need_negative and ontology and negative_policy == "full"
        else []
    )
    synthetic_negatives = (
        ...
        if need_negative
        and ontology
        and negative_policy in {"full", "synthetic_only"}
        else []
    )
    recall_negatives = (
        make_recall_counterfactual_targets(record, rng)
        if need_negative and negative_policy == "full"
        else []
    )
    random_negative = (
        make_random_counterfactual_target(record, ontology, rng)
        if need_negative and ontology and negative_policy == "random"
        else None
    )
    random_negatives = [random_negative] if random_negative else []
    if gate_negatives:
        ...
        selected_negative = random_negatives[0]
    ...
    available_negatives = (
        gate_negatives
        if gate_negatives
        else [*recall_negatives, *synthetic_negatives, *random_negatives]
    )
    return [
        {
            **base,
            ...
            "available_random_negatives": len(random_negatives),
